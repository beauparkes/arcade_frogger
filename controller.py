...
class Player(arcade.Sprite):
    ...
    def __init__(self, filename, scale):
        ...
        if controllers:
            ...
            print("There are no controllers, plug in a controller and run again.")
    ...
    def update(self):
        ...
        if self.controller:
            ...
            # x-axis
            self.change_x = self.controller.x * MOVEMENT_SPEED
            # Set a "dead zone" to prevent drive from a centered controller
            if abs(self.change_x) < DEAD_ZONE:
                self.change_x = 0

            # y-axis
            self.change_y = -self.controller.y * MOVEMENT_SPEED
            # Set a "dead zone" to prevent drive from a centered controller
    ...
    def on_joybutton_press(self, _controller, button):
        """ Handle button-down event for the controller """
        print("Button {} down".format(button))

    # noinspection PyMethodMayBeStatic
    def on_joybutton_release(self, _controller, button):
        """ Handle button-up event for the controller """
        print("Button {} up".format(button))

    # noinspection PyMethodMayBeStatic
    def on_joyhat_motion(self, _controller, hat_x, hat_y):
        """ Handle hat events """
